            ...
            with transaction.atomic():
                ...
                if "products" in data:
                    ...
                    if isinstance(products_raw, str):
                        ...
                        return Response(
                            {"error": "Products must be a JSON string or list"},
                            status=status.HTTP_400_BAD_REQUEST,
                        )

                    if not isinstance(products_data, list):
                        return Response(
                            {"error": "Products must be a list/array"},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                    ...
                    for item in products_data:
                        if isinstance(item, dict):
                            ...
                            flattened_products.extend(
                                [i for i in item if isinstance(i, dict)]
                            )
                            ...
                            try:
                                ...
                                if isinstance(parsed, dict):
                                    ...
                                    flattened_products.extend(
                                        [i for i in parsed if isinstance(i, dict)]
                                    )
                            except json.JSONDecodeError:
                                continue
                ...
                if "checklist_ids" in data:
                    ...
                    if isinstance(checklist_raw, str):
                        try:
                            ...
                            return Response(
                                {"error": "Invalid JSON format for checklist_ids"},
                                status=status.HTTP_400_BAD_REQUEST,
                            )
                            ...
                            if isinstance(item, str):
                                try:
                                    ...
                                    try:
                                        ...
                                        return Response(
                                            {"error": f"Invalid checklist ID: {item}"},
                                            status=status.HTTP_400_BAD_REQUEST,
                                        )
                            else:
                        ...
                        return Response(
                            {"error": "Checklist IDs must be a JSON string or list"},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                    ...
                    try:
                        ...
                        for i, item in enumerate(checklist_ids):
                            ...
                            try:
                                ...
                                return Response(
                                    {
                                        "error": f"Checklist ID at position {i} must be a valid integer. Got: {item}"
                                    },
                                    status=status.HTTP_400_BAD_REQUEST,
                                )
                        ...
                        return Response(
                            {
                                "error": f"Error converting checklist IDs to integers: {str(e)}"
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                ...
                for product_data in products_data:
                    ...
                    product_serializer = UniqueSolarProjectProductSerializer(
                        data=product_data
                    )
                    if not product_serializer.is_valid():
                        raise serializers.ValidationError(
                            f"Product validation error: {product_serializer.errors}"
                        )
                ...
                for image_data in images_data:
                    ...
                    image_serializer = UniqueSolarProjectImageSerializer(
                        data=image_data
                    )
                    if not image_serializer.is_valid():
                        raise serializers.ValidationError(
                            f"Image validation error: {image_serializer.errors}"
                        )
                ...
                for checklist_id in checklist_ids:
                    try:
                        checklist = UniqueSolarChecklist.objects.get(
                            id=checklist_id, is_active=True
                        )
                        ...
                        raise serializers.ValidationError(
                            f"Checklist item with ID {checklist_id} not found or inactive"
                        )

                # Return the complete project data
                complete_serializer = UniqueSolarProjectSerializer(project)
                return Response(
                    complete_serializer.data, status=status.HTTP_201_CREATED
                )
...
class UniqueSolarProjectDetailView(APIView):
    ...
    - PUT/PATCH: Update a unique solar project with nested data
    ...
    def put(self, request, project_id):
        ...
        try:
            with transaction.atomic():
                ...
                # ✅ SAME CHECKLIST LOGIC AS CREATE VIEW
                checklist_ids = []
                if "checklist_ids" in data:
                    checklist_raw = data.pop("checklist_ids")
                    if isinstance(checklist_raw, str):
                        ...
                        checklist_ids = checklist_raw
                    # Convert to integers safely
                    checklist_ids = [int(i) for i in checklist_ids if str(i).isdigit()]
                    # try:
                    #     checklist_ids = [
                    #         int(item)
                    ...
                    # except (ValueError, TypeError) as e:
                    #     return Response(
                    #         {"error": f"Checklist IDs must be valid integers"},
                    #         status=status.HTTP_400_BAD_REQUEST,
                    #     )
                ...
                if "products" in data:
                    ...
                    # Normalize so every element is a dict
                    normalized_products = []
                    for item in products_data:
                        if isinstance(item, dict):
                            ...
                            try:
                                parsed = json.loads(item)
                                if isinstance(parsed, dict):
                                    normalized_products.append(parsed)
                            except json.JSONDecodeError:
                                continue
                ...
                # Update main project
                serializer = UniqueSolarProjectSerializer(
                    project, data=data, partial=True
                )
                if not serializer.is_valid():
                    return Response(
                        serializer.errors, status=status.HTTP_400_BAD_REQUEST
                ...
                # ✅ UPDATE CHECKLIST (MOST IMPORTANT FIX)
                if checklist_ids is not None:
                    # Delete existing checklist items
                    project.checklist.all().delete()  # Uses related_name='checklist'
                    # Create new checklist items
                    for checklist_id in checklist_ids:
                        try:
                            checklist_item = UniqueSolarChecklist.objects.get(
                                id=checklist_id, is_active=True
                            )
                            UniqueSolarProjectChecklist.objects.create(
                                project=project, checklist=checklist_item
                            )
                        except UniqueSolarChecklist.DoesNotExist:
                            continue  # Skip invalid checklist items

                # Return updated project with proper checklist data
                complete_serializer = UniqueSolarProjectSerializer(updated_project)
